...
def main(base_folder, fer_path, ferplus_path):
    ...
    with open(fer_path,'r') as csvfile:
        ...
        for row in islice(fer_rows, 1, None):
            ...
            file_name = ferplus_row[1].strip() #file 이름의 양끝 공백,\n 제거 후 꺼내기
            print('file_name:',file_name)
            if len(file_name) > 0:
                image = str_to_image(row[1])
                if row[0] == '0':
                    image_path = os.path.join(base_folder, folder_names[row[2]], 'Angry', file_name)
                elif row[0] == '1':
                    image_path = os.path.join(base_folder, folder_names[row[2]], 'Disgust', file_name)
                elif row[0] == '2':
                    image_path = os.path.join(base_folder, folder_names[row[2]], 'Fear', file_name)
                elif row[0] == '3':
                    image_path = os.path.join(base_folder, folder_names[row[2]], 'Happy', file_name)
                elif row[0] == '4':
                    image_path = os.path.join(base_folder, folder_names[row[2]], 'Sad', file_name)
                elif row[0] == '5':
                    image_path = os.path.join(base_folder, folder_names[row[2]], 'Surprise', file_name)
                else:
                    image_path = os.path.join(base_folder, folder_names[row[2]], 'Neutral', file_name)
...
if __name__ == "__main__":
    main("data", "D:/python Project/face_recognition/challenges-in-representation-learning-facial-expression-recognition-challenge/fer2013/fer2013.csv",
         "D:/python Project/face_recognition/FERPlus-master/fer2013new.csv")
    '''
